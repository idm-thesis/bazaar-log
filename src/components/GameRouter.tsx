...
const BlueScreen = dynamic(() => import("@/components/BlueScreen"), { ssr: false });
...
export default function GameRouter() {
  ...
  const { currentYear, gameStage, setGameStage, gamePhase, blueScreenCompleted } = useGameStore();
  ...
  useEffect(() => {
    ...
    } else if (currentYear === 2000 && gameStage !== "blueScreen" && !blueScreenCompleted) {
      setGameStage("blueScreen");
    } else if (isInRange(currentYear, 2000, 2010) && gameStage !== "web2000" && blueScreenCompleted) {
    ...
    console.log("[GameRouter] currentYear:", currentYear, "→ gameStage:", gameStage);
  },[currentYear, gameStage, gamePhase]);

  if (!mounted) return null;

  // Testing
  // return (<div className="min-h-screen" style={{background: '#0000ff'}}>
  //   <PreInternet />
  // </div>);
  switch (gameStage) {
    case "preInternet":
